...
namespace Godot.SourceGenerators
{
    ...
    public class ScriptMethodsGenerator : IIncrementalGenerator
    {
        ...
        {
            var areGodotSourceGeneratorsDisabled = context.AnalyzerConfigOptionsProvider.Select(static (provider, _) => provider.IsGodotSourceGeneratorDisabled("ScriptMethods"));
        ...
        )
        {
            ...
            if (godotClassMethods.Length > 0)
            {
                ...
                source.Append("    {\n");
                source.Append("        return ref MethodRegistry.GetMethodOrNullRef(in method, argCount);\n");
                source.Append("    }\n\n");
